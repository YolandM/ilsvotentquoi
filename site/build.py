...
def build_essentiels():
    ...
    rule = ('<p class="hint" style="margin:-6px 0 18px">Règle de sélection, sans choix humain : tous les votes sur l\'ensemble d\'un texte (adoption ou rejet d\'une loi), '
            ...
            f'<a href="/essentiels/motions-de-censure/">Motions de censure seules ({len(motions)})</a></p>')
...
def main():
    ...
    urls = ["/", "/essentiels/", "/essentiels/motions-de-censure/", "/sujets/", "/groupes/", "/deputes/", "/methode/"] + [f"/sujet/{t}/" for t in THEME_LABEL] + [f"/groupe/{g.lower()}/" for g in ORDER] + \
