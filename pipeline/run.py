...
def load_acteurs():
    acts, groups = {}, {}
    # AMO30 (tous les acteurs) d'abord si présent, puis AMO10 (députés actifs) qui a priorité
    for src in ("amo30", "amo"):
        for f in glob.iglob(os.path.join(RAW, src, "**", "acteur", "*.json"), recursive=True):
            a = json.load(open(f, encoding="utf-8"))["acteur"]; i = a["etatCivil"]["ident"]
            m = (a.get("mandats") or {}).get("mandat") or []; m = m if isinstance(m, list) else [m]
            deps_ = [x for x in m if x.get("typeOrgane") == "ASSEMBLEE" and s_(x.get("legislature")) == LEG]
            if src == "amo30" and not deps_: continue
